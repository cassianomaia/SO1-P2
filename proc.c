...
// TASK4 esse processo é igual ao fork, mas chamamos o copyuvm_cow para compartilhamento de paginação
...
procdump(void)                
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    for (i = 0; i < NPDENTRIES; i++){
      /* Verificação se a Page Table Entry (PTE), ou seja, entry, está presente, é uma página do usuário e é acessível */
      if ((pageDir[i] & PTE_U) && (pageDir[i] & PTE_A) && (pageDir[i] & PTE_P)) {
        ...
        /*  Laço para a localização do PPN das tables, ou seja, são percorridas 
            todas as páginas */
    ...
    for (i = 0; i < NPDENTRIES; i++){
      /* Mesma verificação de flags anterior */
      if ((pageDir[i] & PTE_U) && (pageDir[i] & PTE_A) && (pageDir[i] & PTE_P)){
        pde_t * pageTable = P2V(PTE_ADDR(pageDir[i]));
