...
//TASK 3
/* A função recebe como parâmetro a variável de permissão de escrita writeFlag*/
...
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz,uint writeFlag)
{
  ...
  for(i = 0; i < sz; i += PGSIZE) {
	...
	// TASK3 
  /*Se for permitida a escrita, é adicionada a FLAG de escrita PTE_W para todas as paginações pertencentes
    àquele processo, caso contrário, a mesma é removida.*/
	...
	// TASK3 
  /* Além disso, é adicionado um deslocamento para alinhamento da pagina. */
...
copyuvm(pde_t *pgdir, uint sz) {
  ...
  // TASK2 
  /* Como a primeira página da memória está desalocada, é necessário percorrer as paginações 
      do processo pai a partir da segunda pagina, inicializando o contador i com o tamanho de 
      uma página, evitando assim, o acesso ao endereço 0 */
  ...
  for(i = PGSIZE; i < sz; i += PGSIZE){
	  ...
	  // TASK3
    /* Também é necessário modificar para que os processos filhos também recebessem 
